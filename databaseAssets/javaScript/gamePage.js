function updatePageCont() {
    firebase.database().ref('/users/' + GLOBAL_user.uid + '/highscores/' + gameID).once('value', (data) => {
        if (data.val()) {
            ...
            firebase.database().ref('/users/' + GLOBAL_user.uid + '/highscores/' + gameID + '/score').once('value', (data) => {
...
async function displayHighscores(gameID) {
    await firebase.database().ref('/highscores/' + gameID).orderByChild('score').once('value', (data) => {
let highscoreArray=[];
        data.forEach((data2) => {
          highscoreArray.push(data2);
           console.log(data2)
        })
        
         //   let highscore=data2.val().score;
